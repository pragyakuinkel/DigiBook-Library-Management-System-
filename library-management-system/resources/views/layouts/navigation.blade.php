...
<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700" >
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16" style="flex-wrap: wrap"> 
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex" style="flex-wrap: wrap">
                    ...
                    <div class="hidden sm:flex sm:items-center ">
                        <x-dropdown align="right">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link :href="route('selectBook', 'All')">
                                    All
                                </x-dropdown-link>
                                <x-dropdown-link :href="route('selectBook', 'Available')">
                                    Available
                                </x-dropdown-link>
                                <x-dropdown-link :href="route('selectBook', 'Not Available')">
                                    Not Available
                                </x-dropdown-link>
                            </x-slot>
                        </x-dropdown>
                        ...
                        <x-dropdown align="right">
                            ...
                            <x-slot name="content">
                                @foreach($genres as $genre)
                                    <x-dropdown-link :href="route('genreSelect', $genre->id)">
                                        {{$genre->name }}
                                    </x-dropdown-link>
                                @endforeach
                            </x-slot>
                    ...
                    @auth
                        <x-nav-link :href="route('bookHistory')" :active="request()->routeIs('bookHistory')">
                            {{ __('My Books') }}
                        </x-nav-link>
            ...
            <!-- Settings Dropdown -->
            <div class="hidden sm:flex sm:items-center sm:ml-6" style="align-items:center">
                ...
                @endauth
                @guest
                    <a class="main_text_color rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800 w-80 ml-3 mt-1" href="{{ route('login') }}">
                        Login/Register
                    </a>
                @endguest
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('bookHistory')" :active="request()->routeIs('bookHistory')">
                {{ __('My Books') }}
            </x-responsive-nav-link>
            ...
            <div class="sm:hidden block">
                <x-dropdown align="right" class="w-full">
                    <x-slot name="trigger">
                        <button class="w-full text-left inline-flex items-center text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150 p-2">
                            <span>{{ __('Books') }}</span>
        ...
        <form action="{{route('selectBook.search')}}" method="post" class="form-inline mt-2 ml-2">
            @csrf
            <input class="form-control mr-sm-2" type="search" placeholder="Search" aria-label="Search" name="search" id="search" style="width:70%">
            <button class="btn btn-outline-success my-2 my-sm-0 main_color text-white rounded read-more" type="submit" style="width:18%;height:40px"><i class="fa-solid fa-magnifying-glass"></i></button>
        </form>
